...
function alertSearch(id){
  $('#background').show();
  $('#edit_part').show();
  $.get("/search_cpq_detail_by_id",{id:id},function(data){
    var id = data.rows[0].id;
    var cpq_id = data.rows[0].cpq_id;
...
function alertEdit(refresh){
  var id = $('#id').val();
  var cpq_id = $('#cpq_id').val();
  var quantity = $('#quantity').val();
  var product_id = $('#product_id').val();
  var detail = {id:id , cpq_id:cpq_id , quantity:quantity , product_id:product_id , }
  $.post("/update_cpq_detail",{detail : JSON.stringify(detail)},function(data){
    if(data.success){
      alert('修改成功');
      $('#edit_part').hide();
      $('#background').hide();
...
var product_id = $('#product_id').val();
var detail = {id:id , cpq_id:cpq_id , quantity:quantity , product_id:product_id , }
  $.post("/save_cpq_detail",{detail : JSON.stringify(detail)},function(data){
    if(data.success){
      alert('新建成功');
      $('#new_part').hide();
      $('#background').hide();
    ...
    return (
      <div id="edit_part" className="position_absolute animation_one">
        ...
        <div className="edit_part_div">
          ...
          <span className="edit_part_div_input_wrap"><input type="text" className="edit_part_div_input" id="cpq_id"/></span>
        </div>
        <div className="edit_part_div">
        ...
        <div className="edit_part_div">
          <span className="edit_part_div_name">总价:</span>
          <span className="edit_part_div_input_wrap"><input type="text" className="edit_part_div_input" id="total_price"/></span>
        </div>
