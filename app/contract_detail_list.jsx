...
function alertSearch(id){
  $('#background').show();
  $('#edit_part').show();
  $.get("/search_detail_by_id",{id:id},function(data){
    var id = data.rows[0].id;
    var contract_id = data.rows[0].contract_id;
...
var detail  = {id:id, contract_id:contract_id, product_id:product_id, quantity:quantity,}
  $.post("/update_detail",{detail :JSON.stringify(detail)},function(data){
    if(data.success){
      alert('修改成功');
      $('#edit_part').hide();
      $('#background').hide();
...
function alertNew(refresh){
  ...
  var quantity = $('#quantity').val();
  var detail  = {id:id, contract_id:contract_id, product_id:product_id, quantity:quantity,}
$.post("/save_detail",{detail:JSON.stringify(detail)},function(data){
  if(data.success){
    alert('新建成功');
    $('#new_part').hide();
    $('#background').hide();
  ...
  render() {
    return (
      <div id="edit_part" className="position_absolute animation_one">
        ...
        <div className="edit_part_div">
          <span className="edit_part_div_name">合同号:</span>
          <span className="edit_part_div_input_wrap"><input type="text" className="edit_part_div_input" id="contract_id"/></span>
        </div>
        <div className="edit_part_div">
          <span className="edit_part_div_name">产品id:</span>
          <span className="edit_part_div_input_wrap"><input type="text" className="edit_part_div_input" id="product_id"/></span>
        </div>
        <div className="edit_part_div">
  ...
  render() {
    return (
      <div id="new_part" className="position_absolute animation_one">
        ...
        <div className="edit_part_div">
          <span className="edit_part_div_name">合同号:</span>
          <span className="edit_part_div_input_wrap"><input type="text" className="edit_part_div_input" id="new_contract_id"/></span>
        </div>
        <div className="edit_part_div">
          <span className="edit_part_div_name">产品id:</span>
          <span className="edit_part_div_input_wrap"><input type="text" className="edit_part_div_input" id="new_product_id"/></span>
        </div>
        <div className="edit_part_div">
          <span className="edit_part_div_name">数量:</span>
