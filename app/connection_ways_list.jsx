...
function alertSearch(id){
  $('#background').show();
  $('#edit_part').show();
  $.get("/search_by_id",{id:id},function(data){
...
var name = $('#name').val();
$.post("/save_connection_way",{name:name},function(data){
  if(data.success){
    alert('新建成功');
    $('#new_part').hide();
    $('#background').hide();
  ...
  render() {
    return (
      <div id="edit_part" className="position_absolute animation_one">
        ...
        <div className="edit_part_div">
          <span className="edit_part_div_name">联系方式:</span>
